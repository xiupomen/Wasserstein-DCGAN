...
class DCGAN(object):
  ...
  def build_model(self):
    ...
    if self.y_dim:
      ...
      self.D_logits = self.discriminator(inputs, self.y, reuse=False)
      self.sampler = self.sampler(self.z, self.y)
      self.D_logits_ = self.discriminator(self.G, self.y, reuse=True)
    else:
    ...
    # get variables in discriminator for clipping
  ...
  def train(self, config):
    ...
    d_optim = tf.train.RMSPropOptimizer(5e-5).minimize(self.d_loss, var_list=self.d_vars)
    g_optim = tf.train.RMSPropOptimizer(5e-5).minimize(self.g_loss, var_list=self.g_vars)
    
    clip_updates = [w.assign(tf.clip_by_value(w,-self.c,self.c)) for w in self.w_d]
    ...
    # (-0.5,0.5) output looks better than (-1,1)
    ...
    for epoch in xrange(config.epoch):
      ...
      # run d for n_d times against 1 time of g  
      #n_d = 100 if counter < 25 or (counter+1)%500 == 0 else 4
